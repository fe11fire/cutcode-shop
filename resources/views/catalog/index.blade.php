			...
			<section>
				...
				<div class="grid grid-cols-2 sm:grid-cols-3 xl:grid-cols-5 gap-3 sm:gap-4 md:gap-5 mt-8">
					@foreach ($categories as $item)
						@include('catalog.shared.category', ['item' => $item, 'category' => $category])
					@endforeach
						...
						<form action="{{ route('catalog', $category) }}" class="overflow-auto max-h-[320px] lg:max-h-[100%] space-y-10 p-6 2xl:p-8 rounded-2xl bg-card">
							...
							<div>
								...
								<div class="flex items-center gap-3">
									<input name="filters[price][from]" value="{{ request('filters.price.from', 0) }}" type="number" class="w-full h-12 px-4 rounded-lg border border-body/10 focus:border-pink focus:shadow-[0_0_0_3px_#EC4176] bg-white/5 text-white text-xs shadow-transparent outline-0 transition" value="9800" placeholder="От">
									<span class="text-body text-sm font-medium">–</span>
									<input name="filters[price][to]" value="{{ request('filters.price.to', 100000) }}" type="number" class="w-full h-12 px-4 rounded-lg border border-body/10 focus:border-pink focus:shadow-[0_0_0_3px_#EC4176] bg-white/5 text-white text-xs shadow-transparent outline-0 transition" value="142800" placeholder="До">
								</div>
							</div>
							<!-- Filter item -->
							<div>
                                ...
                                <div class="form-checkbox">
									<input name="filters[brands][{{ $brand->id }}]" type="checkbox" id="filters-item-{{ $brand->id }}" value="{{ $brand->id }}" @checked(request('filters.brands.' . $brand->id))>
                            ...
                            @if (request('filters'))
                            <div>
								<a href="{{ route('catalog', $category) }}" type="reset" class="w-full !h-16 btn btn-outline">Сбросить фильтры</a>
							</div>    
                            @endif
						...
						<div class="flex flex-col md:flex-row md:items-center justify-between gap-4 mb-8">
							...
							<div x-data="{}" class="flex flex-col sm:flex-row sm:items-center gap-3">
								...
								<form x-ref="sortForm" action="{{ route('catalog', $category) }}">
									<select name="sort"
                                    x-on:change="$refs.sortForm.submit()" class="form-select w-full h-12 px-4 rounded-lg border border-body/10 focus:border-pink focus:shadow-[0_0_0_3px_#EC4176] bg-white/5 text-white text-xxs sm:text-xs shadow-transparent outline-0 transition">
